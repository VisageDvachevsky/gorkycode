...
> 💼 Для членов жюри подготовлен отдельный пошаговый гайд: [`docs/judges-deployment-guide.md`](docs/judges-deployment-guide.md).
>
> 📦 Перед запуском скопируйте `.env` из архива в корень репозитория (рядом с `Makefile`).
>
> ⚡ Быстрый старт: `./scripts/judge-run.sh` (macOS/Linux) или `pwsh -File .\scripts\judge-run.ps1` (Windows).
...
> ❗ **Нет возможности ставить Docker Desktop?** Minikube может собирать образы самостоятельно. Установите VirtualBox и запускайте `minikube start --driver=virtualbox ...`, а `make` автоматически использует `minikube image build`, если Docker CLI недоступен.
...
```bash
./scripts/setup-check.sh                 # autodetect driver
./scripts/setup-check.sh --driver=virtualbox   # принудительно выбираем VirtualBox
```
...
```
Скрипт создаст `.env.yaml` и `helm/ai-tourist/secrets.yaml` с секцией `secrets:` — Helm подхватит их автоматически.
