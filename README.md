...
- **ADR 0001 — Route optimisation strategy** (`docs/adr/0001-route-optimization.md`): фиксирует переход на эвристику `networkx` с профилями интенсивности и буферами времени для стабильного планирования прогулок.
- **ADR 0002 — Ranking heuristics and category boosts** (`docs/adr/0002-ranking-heuristics.md`): описывает мультипликативные весы категорий и социальные режимы, которые дополняют косинусное сходство.

Обе заметки лежат в `docs/adr/` и служат источником правок для продуктовой и аналитической команд.
...
- Lighthouse-аудит фронта (performance, accessibility, best practices, SEO): `npm run audit:lhci` с конфигурацией `frontend/lighthouserc.js`.
