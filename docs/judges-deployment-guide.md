...
> 🔧 **Для проверки версии PowerShell** выполните: `$PSVersionTable.PSVersion`
...
| Драйвер | ОС | Производительность | Простота | Рекомендация |
|---------|----|--------------------|----------|--------------|
| `docker` | macOS, Linux, Windows+WSL2 | ⚡⚡⚡ Отличная | ⭐⭐⭐ Простая | ✅ Рекомендуется |
| `virtualbox` | Все ОС | ⚡⚡ Средняя | ⭐⭐ Средняя | ⚠️ Для Windows Home |
| `hyperv` | Windows Pro+ | ⚡⚡⚡ Отличная | ⭐⭐ Средняя | ✅ Для Windows без Docker |
| `podman` | Linux | ⚡⚡⚡ Отличная | ⭐⭐ Средняя | ✅ Альтернатива Docker |
...
| **4. Сборка контейнеров** | Сборка Docker-образов внутри Minikube для всех сервисов | `🔧 Building backend...`, `🔧 Building frontend...`, и т.д. | 10-25 мин |
